...
// Mirrors musicTracks.js — frontend is the source of truth
const MUSIC_TRACKS = [
...
async function main() {
  ...
  for (let dayOffset = 0; dayOffset <= 6; dayOffset++) {
    ...
    const { error: songError } = await supabase
      .from('daily_songs')
      .upsert(
        { date: dateStr, song_name: song.name, song_url: song.url, location: Array.isArray(song.location) ? JSON.stringify(song.location) : song.location },
